...
export default function Card({
  ...
  const handleTypeClick = async (clickedType) => {
    setSelectedNoteType(clickedType);
    ...
    <>
      <div className="md:w-80 bg-indigo-600 rounded-lg shadow overflow-hidden hover:bg-indigo-500 py-4 my-8 mx-4 card-wrapper">
        ...
        <span
          ...
          className={`px-2 py-0.5 card-type text-xs font-medium rounded-full mx-3 cursor-pointer ${typeColor(
            noteType
          )}`}
        >
          {noteType}
