...
export default function NoteId({ nnote }) {
  ...
  const [ideaLiked, setIdeaLiked] = useState(false);
  const [sadCount, setSadCount] = useState(0);
  const [sadLiked, setSadLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(0);
  const [likeLiked, setLikeLiked] = useState(false);
  const [loveCount, setLoveCount] = useState(0);
  const [loveLiked, setLoveLiked] = useState(false);
  ...
  useEffect(() => {
    window.localStorage.setItem(`sad-liked-${id}`, false);
    setIdeaLiked(window.localStorage.getItem(`sad-liked-${id}`));
    fetchMyAPI();
  }, [window.localStorage.getItem(`sad-liked-${id}`)]);
  ...
  const handleSadChange = async () => {
    ...
    if (sadLiked == true) {
      ...
      count += 1;
      window.localStorage.setItem(`sad-liked-${id}`, true);
      setSadLiked(true);
    }
    setSadCount(count);
