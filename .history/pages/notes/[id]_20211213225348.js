...
export default function NoteId({ nnote }) {
  ...
  const handleLoveChange = async (data) => {
    let count = loveCount;
    if (count === 0) {
      count = 1;
    } else {
  ...
  const handleIdeaChange = async (data) => {
    let count = ideaCount;
    if (count === 0) {
      count = 1;
    } else {
    ...
    <div className="bg-indigo-800 min-h-screen flex flex-col	justify-center items-center overflow-hidden">
      ...
      <div className="flex mb-4 ">
        ...
        <div className="text-center">
          <BsLightbulbFill
            size={`2em`}
            className="text-indigo-200 cursor-pointer hover:text-indigo-400 mb-3 mx-4 active:bg-indigo-800 focus:outline-none focus:ring focus:ring-violet-300"
