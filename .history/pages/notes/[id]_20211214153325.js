...
export default function NoteId({ nnote }) {
  ...
  useEffect(() => {
    window.localStorage.setItem(`sad-clicked-${id}`, false);
    window.localStorage.setItem(`sad-active-${id}`, false);
    fetchMyAPI();
  }, [`sad-clicked-${id}`, `sad-active-${id}`]);
  ...
  console.log(!window.localStorage.getItem(`sad-active-${id}`));
  const handleSadChange = async () => {
    let count = sadCount;
    let active = !window.localStorage.getItem(`sad-active-${id}`);
    console.log(active);

    let status = window.localStorage.setItem(`sad-active-${id}`, active);
    if (window.localStorage.getItem(`sad-clicked-${id}`) == "false") {
      count += 1;
      setSadClicked(true);
      window.localStorage.setItem(`sad-clicked-${id}`, true);
    } else if (window.localStorage.getItem(`sad-clicked-${id}`) == "true") {
      if (status) {
        count += 1;
        setSadClicked(true);
      } else {
