...
export default function NoteId({ nnote }) {
  ...
  useEffect(() => {
    setSadLiked(localStorage.getItem(`sad-liked-${id}`));

    setLikeLiked(localStorage.getItem(`like-liked-${id}`));

    setLoveLiked(localStorage.getItem(`love-liked-${id}`));

    setIdeaLiked(localStorage.getItem(`idea-liked-${id}`));
