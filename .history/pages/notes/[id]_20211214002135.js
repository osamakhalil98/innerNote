...
export default function NoteId({ nnote }) {
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      ...
      setSadCount(data.sad);
      setLikeCount(data.like);
      setLoveCount(data.love);
      setIdeaCount(data.angry);
    }
  ...
  const handleLoveChange = async () => {
    ...
    await fetch(`/api/notes/${id}`, {
      ...
      body: JSON.stringify({ love: count }),
    });
  };
  ...
  const handleIdeaChange = async () => {
    ...
    await fetch(`/api/notes/${id}`, {
      ...
      body: JSON.stringify({ angry: count }),
    });
  };
  return (
    <div className="bg-indigo-800 min-h-screen flex flex-col	justify-center items-center overflow-hidden">
      ...
      <h3 className="text-indigo-100">Share this innerNote!</h3>
      <div className="rounded-xl bg-indigo-600 shadow-1xl text-indigo-200 mb-4 md:w-80 flex justify-center">
