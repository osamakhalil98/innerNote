  ...
  try {
    ...
    switch (method) {
      case "GET":
        ...
        try {
          const notes = await Note.find({}).skip(0).limit(9).sort({ _id: -1 });
          const pagesNumber = Math.ceil(notes.length / 9);
          res.status(200).json({ success: true, data: notes, pagesNumber });
        } catch (error) {
          console.log("here");
          res.status(400).json({ error }, "This is the problem");
        }
