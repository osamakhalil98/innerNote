...
export default async function handler(req, res) {
  ...
  const { page } = req.query;
    ...
    verify(getCookie('jwt', {req, res}), process.env.JWT_KEY, function(err, decoded) {
      if(!err && decoded){
          res.status(200).json({message:"This is an auth user"});
}
else{
  res.status(401).json({message:"This isn't auth user"})
}})
