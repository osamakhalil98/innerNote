...
export default async function handler(req, res) {
  ...
  const userToken = getCookie('jwt', {req, res});
      ...
      case "GET":
        if (page) {
          ...
          break;
            ...
            const verification=  jwt.verify(userToken, process.env.JWT_KEY, function(err, k){
              if(!err){
                try{
                  const notes = await Note.find({})
                  .skip(0)
            ...
            if(verification){ 
              ...
              res.status(401).json({message:"You're not authorized to see this content"})
            }
